...
func (p *Parser) SetLastEvaluatedT(some any) {
	...
	if p.ErrorRow == p.LspTargetRow {
		switch v := p.lastEvaluatedT.(type) {
		case *base.T:
			p.LspSuggestTargetT = *v
		}
	}
}
...
func (p *Parser) AppendLastReturnT() {
	...
	lastEvaluatedT, ok := p.lastEvaluatedT.(*base.T)
	if !ok || lastEvaluatedT == nil {
		return
	}

	for _, candidateT := range p.lastReturnT {
		if candidateT.IsMatchType(lastEvaluatedT) {
			return
		}
	}
