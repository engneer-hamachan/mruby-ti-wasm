...
func (d *Comma) Evaluation(
	...
	var tArray []*base.T
	if firstT, ok := p.GetLastEvaluatedTPointer().(*base.T); ok {
		tArray = append(tArray, firstT)
	}
	...
	for {
		...
		if nextEvaluatedT, ok := p.GetLastEvaluatedTPointer().(*base.T); ok {
			tArray = append(tArray, nextEvaluatedT)
		}
