...
func printInheritanceMap() {
	for key, parents := range base.ClassInheritanceMap {
		classNode := base.ParseClassNodeKey(key)
		for _, enc := range parents {
			printInheritance(classNode, base.DecodeClassNode(enc))
		}
	}
}
...
func isParentClass(
	...
	for _, enc := range base.ClassInheritanceMap[classNode.Key()] {
		parentNode := base.DecodeClassNode(enc)
...
func PrintTargetClassExtends() {
	...
	for key, parents := range base.ClassInheritanceMap {
		...
		if classNode.Class == className {
			for _, enc := range parents {
				parent := base.DecodeClassNode(enc)
				switch parent.Class {
				case "":
					fmt.Println("Object")
				default:
...
func PrintVersion() {
	fmt.Println("mruby-ti-wasm version " + Version)
}
